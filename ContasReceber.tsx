...
import { ArrowUpCircle, Edit, Trash2, Save, CheckCircle, Filter, RotateCcw } from 'lucide-react';
...
const generateMockContas = (count: number): Conta[] => {
  return Array.from({ length: count }, () => {
    const status = faker.helpers.arrayElement(['Pendente', 'Recebida']);
    return {
      id: faker.string.uuid(),
      description: `Venda - ${faker.commerce.productName()}`,
      amount: parseFloat(faker.finance.amount(80, 2000)),
      dueDate: faker.date.between({ from: new Date(2025, 0, 1), to: new Date(2025, 2, 31) }),
      status: status,
      type: 'Única',
      paymentDate: status === 'Recebida' ? format(faker.date.recent(), 'yyyy-MM-dd') : undefined,
      paymentMethod: status === 'Recebida' ? faker.helpers.arrayElement(['Pix', 'Cartão de Crédito']) : undefined,
    };
  }).sort((a, b) => a.dueDate.getTime() - b.dueDate.getTime());
};

const mockFormasPagamento = ['Dinheiro', 'Pix', 'Cartão de Crédito', 'Cartão de Débito'];
...
const ContasReceber: React.FC = () => {
  const [contas, setContas] = useState<Conta[]>(generateMockContas(12));
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingConta) {
      ...
      showAlert({ title: 'Sucesso', message: 'Nova receita adicionada!', type: 'success' });
    }
    closeModal();
  };
  ...
  const handleConfirmPayment = () => {
    ...
    setContas(contas.map(c => 
      ...
      ? { ...c, status: 'Recebida', paymentDate: paymentInfo.date, paymentMethod: paymentInfo.method } 
      : c
    ));
    setIsConfirmModalOpen(false);
    setConfirmingConta(null);
    showAlert({ title: 'Sucesso', message: 'Recebimento confirmado com sucesso!', type: 'success' });
  };
  ...
  const handleRevertPayment = () => {
    ...
    showConfirm({
      ...
      message: 'Tem certeza que deseja reverter este recebimento? A conta voltará ao status "Pendente".',
      type: 'warning',
      onConfirm: () => {
        setContas(contas.map(c => 
        ...
        closeModal();
        showAlert({ title: 'Sucesso', message: 'Recebimento revertido com sucesso.', type: 'success' });
      }
    });
  };
  ...
  return (
    <div>
        ...
        <table className="w-full text-left min-w-[720px]">
          <thead className="bg-light-200 dark:bg-dark-700">
            <tr>
              ...
              <th className="p-4 text-sm font-semibold text-light-700 dark:text-dark-300 text-center">Receber</th>
          ...
          <tbody>
            {filteredContas.map(conta => (
              <tr key={conta.id} className="border-b border-light-300 dark:border-dark-700 last:border-b-0">
                ...
                <td className="p-4 text-green-500 dark:text-green-400 font-medium">{formatCurrency(conta.amount)}</td>
                <td className="p-4">
                  <span className={`px-2 py-1 text-xs font-medium rounded-full ${conta.status === 'Recebida' ? 'bg-green-100 dark:bg-green-600/20 text-green-800 dark:text-green-400' : 'bg-yellow-100 dark:bg-yellow-600/20 text-yellow-800 dark:text-yellow-400'}`}>
                    {conta.status}
                  </span>
                </td>
                <td className="p-4 text-center">
                  {conta.status === 'Pendente' ? (
                    ...
                    <span className="text-light-500 dark:text-dark-500 text-xs italic">Recebido</span>
                  )}
                </td>
                <td className="p-4">
      ...
      <Modal isOpen={isModalOpen} onClose={closeModal} title={editingConta ? 'Editar Receita' : 'Nova Receita'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center space-x-4">
            <label className="flex items-center space-x-2 cursor-pointer">
              ...
              <span className="text-sm text-light-700 dark:text-dark-300">Receita Recorrente</span>
            </label>
          </div>
          {formData.type === 'Recorrente' && (
            ...
            >
              <div>
                ...
                <p className="text-xs text-light-500 dark:text-dark-500 mt-1">A receita será criada para o mesmo dia nos próximos meses.</p>
              </div>
            </motion.div>
          )}
          <div className="flex justify-between items-center pt-4">
            {editingConta && editingConta.status === 'Recebida' && (
                <button
                  type="button"
                ...
                >
                  ...
                  <span>Reverter Recebimento</span>
                </button>
            )}
      ...
      <Modal isOpen={isConfirmModalOpen} onClose={() => setIsConfirmModalOpen(false)} title="Confirmar Recebimento">
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-light-700 dark:text-dark-300 mb-2">Data do Recebimento</label>
          ...
          <div>
            <label className="block text-sm font-medium text-light-700 dark:text-dark-300 mb-2">Forma de Recebimento</label>
